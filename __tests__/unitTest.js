...
describe('Unit test of format function', () => {
  test('Global hash enabled', () => {
    const { name, cloud_id: cloudId, user, product, subproduct, server, version } = format(
      event,
      options,
    )
  ...
  test('Global hash is disabled', () => {
    const { cloud_id: cloudId, user } = format(event, { ...options, hash: false })
  ...
  test('Global hash is enabled but for current event is disabled', () => {
    const { cloud_id: cloudId, user } = format({ ...event, hash: false }, options)
  ...
  test('Global hash is enabled but for user is disabled', () => {
    const { cloud_id: cloudId, user } = format({ ...event, hash: { user: false } }, options)
  ...
  test("Don't pass user to event, global hashing is still enabled", () => {
    ...
    const { user } = format(restOfEvent, options)
